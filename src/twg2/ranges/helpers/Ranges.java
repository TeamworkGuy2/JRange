...
import twg2.ranges.CharRangeSearcherMutable;
import twg2.ranges.CharRangeSearcherMutableImpl;
import twg2.ranges.IntRange;
import twg2.ranges.IntRangeSearcher;
import twg2.ranges.IntRangeSearcherMutableImpl;
import twg2.ranges.IntSearcher;
...
public final class Ranges {
	 ...
	 * @return true if the ranges overlap, false if they do not overlap
	 */
	public static final boolean doRangesOverlap(float start1, float end1, float start2, float end2) {
		if(start1 > end1 || start2 > end2) {
			throw new IllegalArgumentException("range overlap check, range end is less than range start, " +
					" [" + start1 + ", " + end1 + "], to [" + start2 + ", " + end2 + "]");
		}
	 ...
	 * @return the number of elements that overlap between the two ranges
	 */
	public static final float rangeOverlapCount(float start1, float end1, float start2, float end2) {
		return doRangesOverlap(start1, end1, start2, end2) ? Math.min(end1, end2) - Math.max(start1, start2) + 1 : 0;
	}
	...
	public static final int indexOfRange(CharArrayList rangePoints, char start, char end) {
	...
	public static final int indexOfRange(FloatArrayList rangePoints, float start, float end) {
		for(int index = 0, size = rangePoints.size(); index < size; ) {
			index = rangePoints.indexOf(start, index);
			if (index == -1) {
				break;
	...
	public static final int removeMatchingRangeSections(CharArrayList rangePoints, char rangeStart, char rangeEnd) {
		int rangeCountRemoved = 0;
		for(int i = rangePoints.size() - 1; i > -1; i -= 2) {
			char startA = rangePoints.get(i - 1);
			char endA = rangePoints.get(i);
			if(doRangesOverlap(startA, endA, rangeStart, rangeEnd)) {
				...
				// if the range to remove is contained inside the current range
					...
					rangePoints.set(i, (char)(rangeStart - 1));
					// add a new range for the ending portion of the old range
					rangePoints.add(i + 1, (char)(rangeEnd + 1));
				...
				// if the range to remove overlaps with the bottom of the current range
					...
					rangePoints.set(i - 1, (char)(rangeEnd + 1));
	...
	public static final int removeMatchingRangeSections(FloatArrayList rangePoints, float rangeStart, float rangeEnd) {
		int rangeCountRemoved = 0;
		for(int i = rangePoints.size() - 1; i > -1; i -= 2) {
			float startA = rangePoints.get(i - 1);
			float endA = rangePoints.get(i);
			if(doRangesOverlap(startA, endA, rangeStart, rangeEnd)) {
				// if the range to remove surrounds the current range
				if(rangeStart <= startA && rangeEnd >= endA) {
					rangePoints.remove(i);
	 ...
	 */
	public static final void throwIfRangeExists(CharList rangePoints, char rangeStart, char rangeEnd) {
		// search each range
		for(int i = rangePoints.size() - 1; i > -1; i -= 2) {
	 ...
	 * list must be divisible by 2, every 2 points represent the start and end point of a range
	 */
	public static final void throwIfRangeExists(FloatList rangePoints, float rangeStart, float rangeEnd) {
		// search each range
		for(int i = rangePoints.size() - 1; i > -1; i -= 2) {
			// looking for a duplicate range
			if(rangeStart == rangePoints.get(i - 1) && rangeEnd == rangePoints.get(i)) {
	...
	public static final void addRangeCombineOverlap(CharArrayList rangePoints, char start1, char end1) {
		boolean added = false;
		for(int i = rangePoints.size() - 1; i > 0; i -= 2) {
			char startA = rangePoints.get(i - 1);
			char endA = rangePoints.get(i);
			if(doRangesOverlap(startA, endA, start1, end1)) {
				rangePoints.set(i - 1, (char)Math.min(startA, start1));
				rangePoints.set(i, (char)Math.max(endA, end1));
				added = true;
				break;
			}
	...
	public static final void addRangeCombineOverlap(FloatArrayList rangePoints, float start1, float end1) {
		boolean added = false;
		for(int i = rangePoints.size() - 1; i > 0; i -= 2) {
			float startA = rangePoints.get(i - 1);
			float endA = rangePoints.get(i);
	...
	public static final CharRangeSearcherMutable newCharRangeSearcherMutable() {
		return new CharRangeSearcherMutableImpl();
	}
